...
with open("C:\\Users\\admin\\Downloads\\MultiPurpose-Nextcord\\Music\\SpotipyClientID.txt", "r") as scid:
    spotipy_id = scid.read().strip()
    scid.close()
with open("C:\\Users\\admin\\Downloads\\MultiPurpose-Nextcord\\Music\\SpotipyClientSecret.txt", "r") as scs:
    spotipy_secret = scs.read().strip()
    scs.close()
with open("C:\\Users\\admin\Downloads\\MultiPurpose-Nextcord\\Music\\token.txt", "r") as key:
    ...
    async def audio_player_task(self):
        while True:
            ...
            #If the song is looped
                ...
                self.now = nextcord.FFmpegPCMAudio(self.current.source.stream_url, **YTDLSource.FFMPEG_OPTIONS)
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('This command cant be used in DM channels.')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.voice_state.songs.put(song)
                await ctx.send('Currently playing {}'.format(str(source)))
